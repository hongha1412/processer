...
public class Encrypter {
    ...
    public static String rhex(int num) {
        ...
        for (int j = 0; j <= 3; j++)
            str.append(hex_chr.charAt((int)((num >> (j * 8 + 4)) & 0x0F))).append(hex_chr.charAt((int)((num >> (j * 8)) & 0x0F)));
        return str.toString();
    }

    private static int[] str2blks_MD5(String str) {
        int nblk = ((str.length() + 8) >> 6) + 1;
        int[] blks = new int[(int)(nblk * 16)];
        int i = 0;
        for (i = 0; i < nblk * 16; i++)
            blks[i] = 0;
        ...
        blks[i >> 2] |= 0x80 << ((i % 4) * 8);
        blks[(int)(nblk * 16 - 2)] = str.length() * 8;
        return blks;
    }
    ...
    public static String encrypt(String string) {
        String a = string;
        String b = convertHexToString("a3aca1a3");
