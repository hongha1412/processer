/*!@license
 * Infragistics.Web.ClientUI igGrid KnockoutJS extension 17.1.20171.1012
 *
...
(function(factory) {
    if (typeof define === "function" && define.amd) {
        define(["jquery", "jquery-ui", "knockout", "../modules/infragistics.util", "../modules/infragistics.datasource", "../modules/infragistics.ui.grid.framework"], factory)
    } else {
        factory(jQuery)
    }
})(function($) {
    ...
    ko.bindingHandlers.igHierarchicalGrid = {
        init: function(element, valueAccessor) {
            var grid = $(element),
                binding, options, ds, childgridhandler, key;
            binding = ko.utils.unwrapObservable(valueAccessor());
            options = {};
            ...
            childgridhandler = function(event, args) {
                ...
                ko.applyBindingsToNode(args.element[0], {
                    igGrid: {}
                }, observableDs)
            };
    ...
    ko.bindingHandlers.igCell = {
        update: function(element, valueAccessor, allBindingsAccessor, viewModel) {
            var cell = $(element),
                grid = cell.closest(".ui-iggrid-table");
            if (grid.attr("id").endsWith("_fixed")) {
                grid = grid.closest(".ui-iggrid").find("[data-bind]:first")
            }
            grid.igGridKnockoutBridge("cellUpdated", cell, typeof valueAccessor === "function" ? valueAccessor() : valueAccessor, viewModel !== null && ko.isObservable(viewModel.$data) ? viewModel.$data() : viewModel)
        }
    };
    $.widget("ui.igGridKnockoutBridge", {
        ...
        _rebindCells: function(rowId) {
            var ds, owner = this.options.parent,
                ...
                opts = owner.options,
                dataIdSelector = $.type(rowId) === "number" || $.type(rowId) === "string" ? "[data-id='" + rowId + "']" : "",
                bindingFunction;
            ...
            owner.element.children("tbody").children("tr[data-grouprow!=true][data-container!=true]" + dataIdSelector).filter(function() {
        ...
        _bindingFunctionDefault: function(ds, owner, opts) {
            return function(i) {
                var trId = $(this).attr("data-id"),
                    id, k, l, fl, cells, fcells, dataVal, rec, cell;
                ...
                if (rec) {
                    cells = $(this).children("td[data-parent!=true][data-parent!=false][data-skip!=true]");
                    fcells = $(this).closest(".ui-iggrid").find(".ui-iggrid-fixedcontainer tbody>tr[data-id='" + trId + "']").children("td[data-parent!=true][data-parent!=false][data-skip!=true]");
                    l = 0;
                    fl = 0;
        ...
        _bindingFunctionResponsiveVerticalColumns: function(ds, owner, opts) {
            return function(i) {
                var trId = $(this).attr("data-id"),
                    id, k, cells, dataVal, rec, cell, colkey;
                for (i = 0; i < ds.length; i++) {
                    id = ds[i][owner.options.primaryKey];
                    if (ko.isObservable(id)) {
                        id = id()
        ...
        recordsUpdated: function(valueAccessor) {
            var grid = this.options.parent,
                newds, oldds, updating = $(this.options.parent.element).data("igGridUpdating") || $("#" + grid.id()).data("igGridUpdating"),
        ...
        cellUpdated: function(cell, valueAccessor, viewModel) {
            var grid = this.options.parent,
                key, tmpldata, newFormattedVal, tr, rec, keyVal, col, editor, curCol, origDs, origRec;
