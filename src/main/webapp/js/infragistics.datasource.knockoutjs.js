/*!@license
 * Infragistics.Web.ClientUI igDataSource KnockoutJS extension 17.1.20171.1012
 *
...
(function(factory) {
    if (typeof define === "function" && define.amd) {
        define(["jquery", "jquery-ui", "knockout", "../modules/infragistics.util", "../modules/infragistics.datasource"], factory)
    } else {
        factory(jQuery)
    }
})(function($) {
    $.ig.KnockoutDataSource = $.ig.KnockoutDataSource || $.ig.DataSource.extend({
        ...
        _setCellValue: function(rowId, colId, val) {
            var cellData, pk = this.settings.primaryKey,
                ds = this.kods,
                i;
            ...
            if (pk) {
                for (i = 0; i < ds.length; i++) {
                    ...
                    if (String(cellData) === String(rowId)) {
                        if (ko.isWriteableObservable(ds[i][colId])) {
                            ds[i][colId](val)
                        } else {
                            ds[i][colId] = val
                        }
                ...
                if (ko.isWriteableObservable(ds[rowId][colId])) {
                    ds[rowId][colId](val)
                } else {
                    ds[rowId][colId] = val
                }
        ...
        _deleteRow: function(rowId) {
            var pk = this.settings.primaryKey,
                ds = this.kods;
        ...
        _addRow: function(rowObject, rowId, origDs) {
            var ds = this.kods,
                prop, koObject = jQuery.extend({}, rowObject);
            if (this.settings.responseDataKey && ds[this.settings.responseDataKey]) {
                ds = ds[this.settings.responseDataKey]
            }
            if (ko.isObservable(ds)) {
        ...
        _updateRow: function(rowId, rowObject) {
            ...
            if (pk) {
                    ...
                    if (String(cellData) === String(rowId)) {
                        for (prop in rowObject) {
                            if (rowObject.hasOwnProperty(prop)) {
                                if (ko.isWriteableObservable(ds[i][prop])) {
                ...
                for (prop in rowObject) {
                    if (rowObject.hasOwnProperty(prop)) {
                        if (ko.isWriteableObservable(ds[rowId][prop])) {
            ...
            for (i = 0; i < paths.length; i++) {
                ...
                if (paths[i] !== "") {
                    for (j = 0; data && j < data.length; j++) {
                        ...
                        if (match) {
                            cd = data[j][kp[i]];
                            if (paths.length > 1 && i < paths.length - 1 && this.settings.responseDataKey && $.type(cd) !== "array" && cd[this.settings.responseDataKey]) {
